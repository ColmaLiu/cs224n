...
def negSamplingLossAndGradient(
    ...
    scores = np.matmul(outsideVectors[indices], centerWordVec)
    sigmoid_scores = sigmoid(scores)
    loss = -np.log(sigmoid_scores[0])-np.sum(np.log(1-sigmoid_scores[1:]))
    gradCenterVec = (sigmoid_scores[0]-1)*outsideVectors[outsideWordIdx].T + np.sum(np.multiply(sigmoid_scores[1:],outsideVectors[negSampleWordIndices].T), axis=1)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] += (sigmoid_scores[0]-1)*centerWordVec.T
    for sigmoid_score, negSampleWordIndice in zip(sigmoid_scores[1:],negSampleWordIndices):
        gradOutsideVecs[negSampleWordIndice] += sigmoid_score*centerWordVec.T
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentLoss, currentGradCenterVec, currentGradOutsideVecs = None, None, None
