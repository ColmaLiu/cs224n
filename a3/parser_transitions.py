...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            item2 = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1],item2))
        elif transition == "RA":
            item1 = self.stack.pop(-1)
            self.dependencies.append((self.stack[-1],item1))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for i in range(len(unfinished_parses)-1, -1, -1):
