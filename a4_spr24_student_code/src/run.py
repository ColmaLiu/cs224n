...
if args.function == 'pretrain':
    ...
    ### YOUR CODE HERE ###
    tconfig = trainer.TrainerConfig(max_epochs=650,
                                    batch_size=128, learning_rate=args.pretrain_lr, lr_decay=True, warmup_tokens=512*20,
                                    final_tokens=650*len(pretrain_dataset)*block_size, writer=writer)
    ...
    if args.reading_params_path is not None:  # finetuning with pretrain
        ...
        tconfig = trainer.TrainerConfig(max_epochs=10,
                                        batch_size=256, learning_rate=args.finetune_lr, lr_decay=True, warmup_tokens=512*20,
                                        final_tokens=200*len(pretrain_dataset)*block_size, writer=writer)
    else: # finetuning without pretrain
        tconfig = trainer.TrainerConfig(max_epochs=75,
                                        batch_size=256, learning_rate=args.finetune_lr, lr_decay=True, warmup_tokens=512*20,
                                        final_tokens=200*len(pretrain_dataset)*block_size, writer=writer)
    train_dataset = dataset.NameDataset(pretrain_dataset, open(args.finetune_corpus_path, encoding="utf-8").read())
    Trainer = trainer.Trainer(model, train_dataset, None, tconfig)
    Trainer.train()
    torch.save(model.state_dict(), args.writing_params_path)
    ### END YOUR CODE ###
