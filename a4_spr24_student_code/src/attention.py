...
def precompute_rotary_emb(dim, max_positions):
    ...
    theta = 1 / 10000 ** (-2 * torch.arange(dim//2) / dim)
    positions = torch.arange(max_positions)
    rope_cache[:, :, 0] = torch.cos(torch.outer(positions, theta))
    rope_cache[:, :, 1] = torch.sin(torch.outer(positions, theta))
    ### END YOUR CODE ###
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    rotated_x = None
    ### YOUR CODE HERE ###
    rope = torch.view_as_complex(rope_cache[:x.shape[2], :, :])
    rotated_x = torch.view_as_complex(x.reshape(*x.shape[:-1], -1, 2))
    rotated_x = torch.view_as_real(rope * rotated_x).flatten(3)
    ### END YOUR CODE ###
    return rotated_x
